...
ConfirmStartPage::ConfirmStartPage(QWidget *parent)
    ...
    m_roomInput->setInputMethodHints(Qt::ImhDigitsOnly | Qt::ImhNoPredictiveText);
    m_roomInput->setValidator(new QIntValidator(0, 99999999, m_roomInput));
...
bool ConfirmStartPage::eventFilter(QObject *watched, QEvent *event) {
    if (watched == m_roomInput && event->type() == QEvent::MouseButtonPress) {
        NumericInputDialog dialog("Nhập số phòng", m_roomInput->text(), 8, false, this);
        if (dialog.exec() == QDialog::Accepted) {
            m_roomInput->setText(dialog.value());
        }
        return true;
    }
